...
import { LibrarySymbolInfo, SubscribeBarsCallback } from '../../public/static/charting/charting_library';
import { logMessage } from './helpers';
import { GetBarsResult, HistoryProvider } from './HistoryProvider';
...
export class DataPulseProvider {
	...
	public constructor(private readonly historyProvider: HistoryProvider, updateFrequency: number) {
	...
	private _updateDataForSubscriber(listenerGuid: string): Promise<void> {
		...
		const rangeEndTime = parseInt((Date.now() / 1000).toString(), 10);
