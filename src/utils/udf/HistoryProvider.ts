...
import { Bar, HistoryMetadata, LibrarySymbolInfo, PeriodParams } from '../../public/static/charting/charting_library/charting_library';
import { RequestParams, UdfErrorResponse, UdfOkResponse, UdfResponse } from './helpers';
import { Requester } from './Requester';

export interface HistoryPartialDataResponse extends UdfOkResponse {
	t: any;
	c: any;
	o?: never;
	h?: never;
...
export interface HistoryNoDataResponse extends UdfResponse {
	s: 'no_data';
	nextTime?: number;
}
...
export class HistoryProvider {
	public constructor(private readonly datafeedUrl: string, private readonly requester: Requester) {}
	...
	public getBars(symbolInfo: LibrarySymbolInfo, resolution: string, periodParams: PeriodParamsWithOptionalCountback): Promise<GetBarsResult> {
		...
		return new Promise((resolve: (result: GetBarsResult) => void, reject: (reason: string) => void) => {
			this.requester
				.sendRequest<HistoryResponse>(this.datafeedUrl, 'history', requestParams)
