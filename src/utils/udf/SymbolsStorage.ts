import { definedValueOrDefault, getErrorMessage } from '@koyofinance/core-sdk';
import { LibrarySymbolInfo, ResolutionString, SearchSymbolResultItem, VisiblePlotsSet } from '../../public/static/charting/charting_library';
import { logMessage } from './helpers';
import { Requester } from './Requester';

export interface SymbolInfoMap {
	[symbol: string]: LibrarySymbolInfo | undefined;
}

export interface ExchangeDataResponseSymbolData {
	type: string;
...
function symbolKey(symbol: string, currency?: string, unit?: string): string {
	return `${symbol}${currency ? `_%|#|%_${currency}` : ''}${unit ? `_%|#|%_${unit}` : ''}`;
}
...
export class SymbolsStorage {
	...
	public constructor(datafeedUrl: string, datafeedSupportedResolutions: ResolutionString[], requester: Requester) {
		...
		this._readyPromise.catch((error: Error) => {
			...
			console.error(`SymbolsStorage: Cannot init, error=${error.message}`);
		});
	}
