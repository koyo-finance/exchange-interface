...
export interface MobileNavProps {
	...
	tradeOpen: boolean;
	setTradeOpen: (state: boolean) => void;
}

const MobileNav: React.FC<MobileNavProps> = ({ exchangeOpen, setExchangeOpen, kyoOpen, setKyoOpen, tradeOpen, setTradeOpen }) => {
	return (
			...
			<div className="w-full space-y-1 px-2 pt-4 pb-3 shadow-lg">
				<div className=" flex w-full justify-center">
					<div className=" flex flex-col gap-y-6 text-center text-white">
						...
						<div className={`relative z-20 w-full ${kyoOpen ? ' mb-44' : ''} transform-gpu duration-150`}>
						...
						<div className={`relative z-20 w-full ${tradeOpen ? ' mb-12' : ''} transform-gpu duration-150`}>
							<div className=" flex w-full flex-row justify-center gap-1" onClick={() => setTradeOpen(!tradeOpen)}>
								<div>Trade</div>
								<div>
									<RiArrowDownSLine className={` ${tradeOpen ? 'rotate-180' : 'rotate-0'} transform-gpu text-xl duration-150 `} />
								</div>
							</div>
							<div
								className={` ${
									tradeOpen ? '' : ' hidden opacity-0'
								} absolute top-10 left-0 flex w-full transform-gpu flex-col gap-y-4 duration-150`}
							>
								<div className="z-20 w-full rounded-2xl py-2">
